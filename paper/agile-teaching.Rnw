...
\item Flipped learning \cite{karabulut2018systematic} over traditional lecture-based learning. Flipped learning {\em flips} the concept of class and classroom, making the classroom a space for student-centered, autonomous learning, with the teacher providing material and guidance. However, this concept by itself does not constrain the kind of material the teacher will provide or how this will be sequencer or organized. We need additional best practices in this area, like the ones below.
\item Project-based learning \cite{krajcik2006project,7274311} over traditional lecture-based learning. It gives the student autonomy (which is one of the items in the above mentioned manifesto) through the development of a project across one or several subjects, as opposed to being the receptors of an one-way flow of information.

\item formative evaluation \cite{tessmer1994formative} also fits the autonomous learning part of the manifesto, but also focuses on learning objectives, since it pursues helping the student through continuous feedback on their submitted work (that advances the project mentioned above). This feedback helps continuous improvement of the project they are working with, which is other of the items in the manifesto. Finally, {\em flipped learning} \cite{otero2018flipped}, that turns classrooms into working, and not student-listening-to-lectures spaces, is another best practices that blends well with the two mentioned above.
\end{itemize}
